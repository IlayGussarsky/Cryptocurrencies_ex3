...
wallet_bytecode, wallet_abi = compile(
    r'C:\Users\97250\Documents\exricises\Cryptocurrencies\git_exercise4\ex4_files\part1\VulnerableWallet.sol')
attack_bytecode, attack_abi = compile(
    r'C:\Users\97250\Documents\exricises\Cryptocurrencies\git_exercise4\ex4_files\part1\WalletAttack.sol')
...
class TestWalletAttack(unittest.TestCase):
    def test_wallet_attack_adi(self):
        # Deploy VulnerableWallet contract
        vulnerable_wallet_interface = w3.eth.contract(abi=wallet_abi, bytecode=wallet_bytecode)
        ...
        # Call exploit function in WalletAttack contract
        gas_estimate = wallet_attack_instance.functions.exploit(vulnerable_wallet_address).estimate_gas(
            {'from': accounts[2], 'value': w3.to_wei(1, 'ether')})
        ...
        assert attacker_balance_after_attack == 0

        # Check the balance of accounts[2] after the attack
        account2_balance_after_attack = get_balance(accounts[2])
    ...
    def test_wallet_has_less_than_3_ether(self):
        ...
        # Check the balance of accounts[2] after the attack
        # Deposit 1 Ether to VulnerableWallet
        tx_hash = vul_wallet_instance.functions.deposit().transact({'from': accounts[0], 'value': w3.to_wei(1, 'ether')})
        w3.eth.wait_for_transaction_receipt(tx_hash)
        assert get_balance(vulnerable_wallet_address) == 1

        # Attempt to exploit the WalletAttack contract on the vulnerable wallet
        with self.assertRaises(ContractLogicError):
            wallet_attack_instance.functions.exploit(vulnerable_wallet_address).transact(
                {'from': w3.eth.accounts[2], 'value': w3.to_wei(1, 'ether')})
